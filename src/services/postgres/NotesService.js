...
const myDBToModel = require('../../utils');
...
class NotesService{
  ...
  async getNote(owner){
    try {
      ...
      const result = await this._pool.query(query);
      const mappedResult = result.rows.map(myDBToModel);
  ...
  async getNoteById(id){
    ...
    return result.rows.map(myDBToModel)[0];
  }
    ...
    try {
      ...
      if (error instanceof NotFoundError) {
        ...
        try {
          ...
          throw new AuthorizationError('Anda tidak berhak mengakses resource ini');x;
        }
